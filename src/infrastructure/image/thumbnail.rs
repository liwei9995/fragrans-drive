...
pub fn generate_thumbnail(data: &[u8]) -> Result<Vec<u8>, AppError> {
    ...
    let dimensions = reader.into_dimensions().map_err(|e| AppError::BadRequest(format!("Cannot read dimensions: {}", e)))?;
    if dimensions.0 > 20000 || dimensions.1 > 20000 {
        return Err(AppError::BadRequest("Image dimensions exceed 20000 pixels".into()));
    }
    if (dimensions.0 as u64) * (dimensions.1 as u64) > 100_000_000 {
        return Err(AppError::BadRequest("Image exceeds 100,000,000 pixels".into()));
    }
