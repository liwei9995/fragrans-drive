...
test.describe('Storage E2E', () => {
  ...
  test('should handle context menu, folder navigation, rename, move, and delete', async ({
    page,
  }) => {
    ...
    await page
      .locator('.el-dropdown-menu__item:visible', { hasText: '重命名' })
      .click()
    ...
    await expect(
      page.locator(`.card-container:has-text("${renamedFolder}")`),
    ).toBeVisible()
    await expect(
      page
    ...
    const renamedCard = page.locator(
      `.card-container:has-text("${renamedFolder}")`,
    )
    ...
    await expect(page.locator('.breadcrumb-wrapper:visible')).toContainText(
      renamedFolder,
    )
    ...
    // 5. Move
    await page
      .locator(`.card-container:has-text("${renamedFolder}")`)
      .click({ button: 'right' })
    await page
    ...
    await page
      ...
      .click() // just cancel for now to test dialog appears
    ...
    await page
      ...
      .click({ button: 'right' })
    await page
      .locator('.el-dropdown-menu__item:visible', { hasText: '删除' })
      .click()
