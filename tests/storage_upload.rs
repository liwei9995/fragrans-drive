...
async fn parent_id_order_does_not_change_destination() {
    ...
    let folder_req = Request::builder()
        ...
        .body(Body::from(r#"{"name": "test_parent", "parentId": "root", "type": "folder"}"#))
        .expect("request");
    
    let res = ctx.app.clone().oneshot(folder_req).await.expect("create folder");
    assert_eq!(res.status(), StatusCode::OK);
    ...
    // File field FIRST
    body.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
    body.extend_from_slice(b"Content-Disposition: form-data; name=\"file\"; filename=\"file.txt\"\r\n");
    body.extend_from_slice(b"Content-Type: application/octet-stream\r\n\r\n");
    ...
    let path_nodes: Vec<serde_json::Value> = serde_json::from_slice(&response_bytes(path_res).await).unwrap();
    assert!(path_nodes.iter().any(|n| n["id"].as_str().unwrap() == parent_id));
...
async fn malformed_multipart_returns_400() {
    ...
    let req = Request::builder()
        ...
        .uri("/v1/storage/upload")
        .header(header::AUTHORIZATION, format!("Bearer {}", ctx.auth_token))
        .header(header::CONTENT_TYPE, "multipart/form-data; boundary=invalid")
